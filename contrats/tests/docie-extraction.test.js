...
test("isEnabled / isEligible / voiePour: flag parsing et portée par pièce", () => {
  ...
  // fiscale aussi (#215), voie TEXTE : son schéma et son mapping existaient
  // déjà, seul le câblage manquait.
  assert.equal(isEligible([{ id: "fiscale" }]), true);
  assert.equal(voiePour([{ id: "fiscale" }]), "texte");
  // Les trois autres pièces ne sont jamais envoyées d'ici. coordonnees et
  // specifique n'ont ni schéma ni mapping (champs de saisie) ; cni en a, mais
  // relève de la voie vision du catalogue (prérequis MRZ, lib/mrz.js).
  for (const id of ["cni", "coordonnees", "specifique"]) {
  ...
  assert.deepEqual(Object.keys(VOIES).sort(), ["fiscale", "kbis", "rib", "urssaf"]);
...
test("flag on but item non éligible (ex: cni): reste local (voie vision, non câblée ici)", async () => {
  let localCalls = 0;
  const analyzeLocal = async (body) => { localCalls++; return fakeLocal("local")(body); };
  ...
  const body = { dataBase64: "AA==", mimeType: "application/pdf", items: [{ id: "cni" }] };
